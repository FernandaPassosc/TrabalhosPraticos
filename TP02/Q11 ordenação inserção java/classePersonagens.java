...
{  
   ...
   /*
    * Esse metodo vai fazer uma ordenação por inserção
    */
   public static void OrdenacaoInsercao (Personagens[] P, int n) throws Exception
   {
         for (int i = 0; i < n ; i++)
         {
            ...
            aux = P[i].Clone();
            int j = i - 1;
            String menor = P[i].getanoNascimeto();
        //    int retorno = menor.compareTo(P[j].getanoNascimeto());
            while (j >= 0 && (P[j].getanoNascimeto().compareTo(menor) > 0))  
            {  
               if (P[i].getanoNascimeto().compareTo("unknow") == 0)
               {
