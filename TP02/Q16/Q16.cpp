...
Personagem Maior (Personagem P1, Personagem P2);
void Bolha (Personagem P[]);
...
/*
 * Essa função vai orderna meu array de personagens
 */
void Bolha (Personagem P[])
{
   int i;
   ...
   for (i = (n - 1); i > 0; i--)
   {
      for (j = 0; j < i; j++)
      {
         Personagem tmp = Maior(P[j], P[j+1]);
         
         if (strcmp(tmp.ano_aniversario, P[j].ano_aniversario) == 0)
         {
            swap (P, j, j+1);
         }
      }
...
{
   Personagem aux;
   if (strcmp (P1.ano_aniversario, P2.ano_aniversario) > 0)
   {
      aux = P1;
   }
   else
...
void Desempate(Personagem P[])
{
   ...
   for (int i = 0; i < n; i++)
   {
      if (strcmp(P[i].ano_aniversario, P[i+1].ano_aniversario) == 0 && (i < n -1))
      {
         PosI = i;

         while (strcmp(P[i].ano_aniversario, P[i+1].ano_aniversario) == 0)i++;
