...
void Imprimir (Personagem P[]) 
{
   for (int i = 0; i < 10; i++)
   {
      printf (" ## %s ## ", array[i].nome);
		printf ("%i ## ", array[i].altura);
		printf ("%g ## ", array[i].peso);
		printf ("%s ## ", array[i].cor_do_cabelo);
		printf ("%s ## ", array[i].cor_da_pele);
		printf ("%s ## ", array[i].cor_dos_olhos);
		printf ("%s ## ", array[i].ano_aniversario);
		printf ("%s ## ", array[i].genero);
		printf ("%s ## ", array[i].home_world);
		printf ("\n");	
   }
}
...
void constroi (int tam)
{
   for (int i = tam; i > 1 && array[i].altura > array[i/2].altura; i /= 2)
   {
      swap(array, i, i/2);
   }
}
...
void reconstroi (int tam)
{
   ...
   while (i <= (tam/2))
   {
      if(array[2*i].altura > array[2*i+1].altura || 2*i == tam)
      {
         filho = 2*i;
...
void Decrescente (Personagem P[])
{
  /*
   ...
   for (int i = 1; i <= n; i++)
   {
      ...
      while (j >= inicio && (P[j].altura < menor))
      {
         array[j+1] = P[j];
         j--;
      }
...
void Desempate(Personagem P[])
{
   ...
   for (int i = 0; i < n; i++)
   {
      if ((array[i].altura == array[i+1].altura) && (i < n -1))
      {
         PosI = i;

         while (array[i].altura == array[i+1].altura)i++;
...
void Insercao(Personagem P[], int inicio, int fim)
{
   ...
   for (int i = inicio; i <= fim; i++)
   {
      aux = array[i];
      int j = i -1;
      strcpy(menor, array[i].nome);

      while (j >= inicio && (strcmp(array[j].nome, menor) > 0))
      {
         array[j+1] = array[j];
         j--;
      }
